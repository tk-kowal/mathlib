...
namespace testing
{
    ...
    {
        auto other = tml::vec4{0.07, -1.29, 3.99, -20};
        auto vec = tml::vec4(other);
    ...
    {
        auto a = tml::vec4(1.7, 2.8, 3.33, -20);
        auto b = 2.25f;
        auto c = tml::vec4(3.95, 5.05, 5.58, -17.75);
        EXPECT_TRUE((a + b) == c);
    }

    TEST(Vec4Test, SubtractScalar)
    {
        auto a = tml::vec4(1.7, 2.8, 3.33, -19);
        auto b = 1.0f;
        auto c = tml::vec4(0.7, 1.8, 2.33, -20);
        EXPECT_TRUE((a - b) == c);
    }

    TEST(Vec4Test, MultiplyScalar)
    {
        auto a = tml::vec4(1.7, 2.8, 3.33, -4);
        auto b = 2.0f;
        auto c = tml::vec4(3.4, 5.6, 6.66, -8);
        EXPECT_TRUE((a * b) == c);
    }

    TEST(Vec4Test, DivideScalar)
    {
        auto a = tml::vec4(2.5, 5.0, -10, -15);
        auto b = 2.0f;
        auto c = tml::vec4(1.25, 2.5, -5, -7.5);
        EXPECT_TRUE((a / b) == c);
    }

    TEST(Vec4Test, AddVector)
    {
        auto a = tml::vec4(1.7, 2.8, 3.33, 123);
        auto b = tml::vec4(4.2, -5, 6.0, 1.5);
        auto c = tml::vec4(5.9, -2.2, 9.33, 124.5);
        EXPECT_TRUE((a + b) == c);
    }

    TEST(Vec4Test, SubtractVector)
    {
        auto a = tml::vec4(1.07, 5, -4.5, 100);
        auto b = tml::vec4(1);
        auto c = tml::vec4(0.07, 4, -5.5, 99);
        EXPECT_TRUE((a - b) == c);
    }

    TEST(Vec4Test, MultiplyVector)
    {
        auto a = tml::vec4(1.07, 5, -4.5, 17);
        auto b = tml::vec4(-2);
        auto c = tml::vec4(-2.14, -10, 9, -34);
        EXPECT_TRUE((a * b) == c);
    }

    TEST(Vec4Test, DivideVector)
    {
        auto a = tml::vec4(10, 5, -8, 22);
        auto b = tml::vec4(2);
        auto c = tml::vec4(5, 2.5, -4, 11);
        EXPECT_TRUE((a / b) == c);
    }

    TEST(Vec4Test, DotProduct)
    {
        auto a = tml::vec4(1, 2, 3, 4);
        auto b = tml::vec4(5, 6, 7, 8);
        EXPECT_TRUE(AreEq(70, tml::dot(a, b)));
    }

    TEST(Vec4Test, MagnitudeDefault)
    {
        auto a = tml::vec4(0, 0, 0, 0);
        EXPECT_EQ(0, tml::mag(a));
    }

    TEST(Vec4Test, MagnitudeUnit)
    {
        auto a = tml::vec4(1, 1, 1, 1);
        EXPECT_TRUE(AreEq(std::sqrt(4), tml::mag(a)));
    }
    ...
    {
        auto a = tml::vec4(1, 2, 3, 4);
        auto e = tml::vec4(0.18257, 0.36515, 0.54772, 0.73030);
